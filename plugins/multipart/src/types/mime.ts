/**
 * Generated by scripts/generate-mime-types.ts from IANA's official
 * per-category media-type registries (https://www.iana.org/assignments/media-types).
...
export type KnownMimeType = ApplicationMimeType | AudioMimeType | FontMimeType | ImageMimeType | MessageMimeType | ModelMimeType | TextMimeType | VideoMimeType

/**
 * All IANA-registered MIME types, plus an escape hatch (`string & {}`) so
 * custom or vendor-specific MIME types not in the registry still type-check.
 * The intersection with `{}` keeps literal-string autocomplete for the known
 * cases working in editors that support it.
 */
export type MimeType = KnownMimeType | (string & {})
